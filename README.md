...
### NordVPN
NordVPN is a paid service and requires an OpenVPN config file (`.ovpn`) that must be downloaded from the [NordVPN dashboard](https://my.nordaccount.com/).  The `networkmanager-openvpn` plugin enabled in `nixos/configuration.nix` lets you import NordVPN profiles and toggle them from the GNOME system menu.
